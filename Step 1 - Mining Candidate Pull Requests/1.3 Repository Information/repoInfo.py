...
def get_repositories():
    ...
    query = """ select owner_repo 
                from repositories where stars is null and error is null limit 5000
            """
...
def update_error(repository, error):
    db = get_connection()
    cursor = db.cursor()
    ...
    try:
        ...
        cursor.execute(sql, (str(error), repository))
        db.commit()
...
def process_repository(repository): 
    ...
    if 'errors' in result:
        update_error(repository,f"Error in response for {repository}: {result['errors']}")
        print_with_timestamp(f"Error in response for {repository}: {result['errors']}")
        return
    
    if result.get('data', {}).get('repositoryOwner') is None:
        update_error(repository,f"No repository owner data found for {repository}.")
    ...
    try:
        ...
        primary_language = result["data"]["repositoryOwner"]["repository"]["primaryLanguage"]
        ...
        update_error(repository,f"Failed to import {repository}: {e}. Additional details: {error_detail}")
