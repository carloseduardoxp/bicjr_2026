...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        File rootDir = new File(parentDirectory, "output_classes");

        File[] projectsDir = rootDir.listFiles();
        Arrays.sort(projectsDir, Comparator.comparingInt(
        ...
        for (File projectDir : projectsDir) {
            ...
            if (countProjects >= 874) {
                main.processBaseline(projectDir);                    
            }            
            System.out.println("Finished processing project " + countProjects + " ------- " + projectDir.getName());
            countProjects++;
        }
    }
